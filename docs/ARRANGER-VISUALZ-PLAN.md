...
| Export | **Primary: Export MP4** = `exportMp4` / `exportVisWithA1` in `app/src/export/vis-export.ts`. Tauri/Windows: vis-only temp + A1 range WAV → ffmpeg `-map 0:v:0 -map 1:a:0 -c:v copy -c:a aac -b:a 320k -shortest`. Probe (ffprobe `codec_type=audio` and/or `mp4HasSoundTrack`); no audio stream → FAIL, never Fertig. ffmpeg missing → hard fail (not silent vis-only). Browser: WebCodecs AAC, same probe. Same `resolveExportRange`. Fertig: `… · audio: aac · from A1 · range …`. **Secondary:** Export vis-only (no audio). |
| LEXI Terrain Gold | Own family `LEXI Terrain Gold`, id `lexi-terrain-gold`, mode `loop-seamless`. Pass 1+2: continuous dune surface + depth fog (not line/particle soup). Phase `fract(timeSec / 12)`. Audio listen ~0.07. Bloom ≤ 0.55. Pass 3+ deferred. Defaults: surfaceDensity 1.4, depthAttenuation 0.62, flowSpeed 0.35, fogDensity 0.78, fogHeight 0.38, bloomCap 0.55, horizonY 0.42, mountainScale 1.15, periodSec 12. |

/**
 * LEXI Terrain Gold · Pass 1+2 (clock) — retained stage.
 * Recovered from commit 87d630b. Time-driven u / parx / motes + light bass ampLift.
 * New LEXI stages get new ids; this file is never overwritten by later stages.
 */
...
export const LEXI_TERRAIN_GOLD_P12_ID = "lexi-terrain-gold-p12";
export const LEXI_TERRAIN_GOLD_P12_FAMILY = "LEXI Terrain Gold";
export const LEXI_TERRAIN_GOLD_P12_MODE = "loop-seamless";
export const LEXI_TERRAIN_GOLD_P12_PERIOD_SEC = 12;
...
export const LEXI_TERRAIN_GOLD_P12_PASS = { implemented: [1, 2] as const, deferred: [3] as const };

/** Named defaults — mapped into SceneParams and the catalog entry. */
export const LEXI_TERRAIN_GOLD_P12_DEFAULTS: SceneParams = {
  intensity: 0.86,
  ...
  horizonY: 0.42,
  mountainScale: 1.15,
  periodSec: LEXI_TERRAIN_GOLD_P12_PERIOD_SEC,
};
...
export function terrainPhase(timeSec: number, periodSec = LEXI_TERRAIN_GOLD_P12_PERIOD_SEC): number {
  const T = Math.max(1e-6, periodSec);
  const u = timeSec / T;
  return u - Math.floor(u);
}

export function terrainCappedBloom(params: Partial<SceneParams> = LEXI_TERRAIN_GOLD_P12_DEFAULTS): number {
...
export function terrainMatterPlan(
  width: number,
  height: number,
  params: Partial<SceneParams> = LEXI_TERRAIN_GOLD_P12_DEFAULTS,
): {
  horizonY: number;
  slices: number;
  cols: number;
  ...
  return {
    ...
    periodSec: num(params.periodSec, LEXI_TERRAIN_GOLD_P12_PERIOD_SEC),
...
export const lexiTerrainGoldP12Scene: Scene = {
  id: LEXI_TERRAIN_GOLD_P12_ID,
  name: "Terrain Gold · Pass 1+2 (clock)",
  description: "Pass 1+2 clock-travel dunes — retained stage, not MotionGate",
  defaultParams: LEXI_TERRAIN_GOLD_P12_DEFAULTS,
  render(ctxWrap: SceneContext, features: AudioFeatures, params: SceneParams) {
    ...
    const period = num(params.periodSec, LEXI_TERRAIN_GOLD_P12_PERIOD_SEC);
